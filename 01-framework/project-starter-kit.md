...
20. [Agentic Industry Standards](#agentic-industry-standards)
21. [Final Rules — Non-Negotiable](#final-rules--non-negotiable)
...
- Verify by doing the real thing, not by simulating success — see global-rules.md's Verification Philosophy. For UI-driven features, that means exercising the actual DOM/components a user would click, not just asserting code compiles.
...
> This framework ships a **worked example** of a 9-step (Step 0–8) AI-agent-driven E2E QA pipeline powered by official Playwright Test Agents and Playwright MCP.
> The example lives at [`04-worked-example/`](../04-worked-example/) (sibling to `01-framework/`, at the framework root) — see [`how-this-example-applies-the-framework.md`](../04-worked-example/how-this-example-applies-the-framework.md) for a guided walkthrough. Adapt it to your own application rather than treating it as fixed.

#### Overview

The agentic workflow converts a user story into committed, healed Playwright tests through automated steps:

```
[Step 0: Setup] → User Story → Test Plan → Manual Exploration → Script Generation → Execute & Heal → Report → [Step 8: Audit] → Git Commit
```
...
Use `/init` (see [`03-setup/setup-guide.md`](../03-setup/setup-guide.md)) to scaffold the worked example into your target project, then run its installer:

```powershell
# Script auto-detects your platform (opencode/claude/vscode)
# and installs: @playwright/mcp + playwright init-agents
.\scripts\install-playwright-agents.ps1
```

```bash
# Bash equivalent
bash scripts/install-playwright-agents.sh
```
...
4. **Trigger:** The AI agent discovers `WORKFLOW_MANIFEST.json`, hydrates `TEMPLATE_prompt_workflow.md` from `qa-config.json`, executes Step 0 (GitHub + .env check), then runs the full pipeline (Steps 1–7 + Step 8 Audit)
5. **Outputs produced:**
...
| Resource | Location |
|----------|----------|
| Workflow manifest | [`04-worked-example/WORKFLOW_MANIFEST.json`](../04-worked-example/WORKFLOW_MANIFEST.json) |
| Template prompt | [`04-worked-example/TEMPLATE_prompt_workflow.md`](../04-worked-example/TEMPLATE_prompt_workflow.md) |
| Config template | [`04-worked-example/QA_CONFIG_TEMPLATE.json`](../04-worked-example/QA_CONFIG_TEMPLATE.json) |
| Example output | [`04-worked-example/AgentE2EQAWorkflow-Playwright-main/`](../04-worked-example/AgentE2EQAWorkflow-Playwright-main/) |
...
When a step fails, diagnose using the Root-Cause-In-Context Principle
(see agents.md) — find the stale/wrong input or context that produced
the failure before patching the symptom.
```
...
> **Session bootstrap procedure has moved.** It used to be duplicated here as "SESSION START PROTOCOL"; it now lives in exactly one place — [`session-start-checklist.md`](session-start-checklist.md) — to avoid the two files drifting out of sync. Read that file first, every session.
...
| Diagnose root cause in context, not just the symptom | See `agents.md`'s Root-Cause-In-Context Principle — fix the stale/wrong input that produced the failure |
...
*Consolidated into the Agentic Coding Framework — see [`../README.md`](../README.md)*
